      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          <motion.div 
            ...
            className="pb-4"
          >
            <Link href="#methodology" className="text-primary font-bold border-b border-primary/20 pb-1 hover:border-primary transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8">
          {solutions.map((item, i) => (
            <motion.div 
              ...
              className={`group p-8 bg-surface border border-white/5 rounded-3xl hover:border-primary/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(197,160,89,0.05)] ${item.translateClass}`}
            >
              {/* Note: The stagger effect (translateY) from the original HTML is replaced by a simpler hover lift for better React hydration, but we can keep the staggered layout with custom CSS if needed. For now, matching the base aesthetic. */}
              
              <div className="w-12 h-12 bg-amber-500/10 rounded-2xl flex items-center justify-center text-primary mb-8 group-hover:scale-110 transition-transform duration-500">
                {item.icon}
              </div>
